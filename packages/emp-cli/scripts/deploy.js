...
module.exports = async ({ts, createName, createPath}) => {
  ...
  pathList.forEach((i) => {
    ...
    if (i.path.split('/').length > 1) {
      // 如果是全路径
      shell = `cp -r ${resolveApp(createPath || 'dist')}/${createName || 'index'}.d.ts ${i.path}\/src\/${package.name}.d.ts`;
    } else {
      // 如果只有文件夹名称，默认是和当前目录同层级的
      shell = `cp -r ${resolveApp(createPath||'dist')}/${createName||'index'}.d.ts ${pwd()}\/${i.path}\/src\/${package.name}.d.ts`;
    };
